SELECT * FROM (
    -- based off the explore https://analytics.gov.bc.ca/explore/snowplow_web_block/searches?qid=J39QHvITildRxy69Nd92cA&origin_space=37&toggle=fil,vis 
    SELECT
    ...
    WHERE (searches.collector_tstamp ) < ((DATEADD(day,-1, DATE_TRUNC(''day'',GETDATE()) ))) AND LENGTH(searches.terms ) <> 0 AND ((COALESCE(cmslite_themes.topic, ''(no topic)'') ) = ''Service BC'' AND ((( searches.node_id ) ILIKE  ''%'') AND (( searches.page_urlhost  ) ILIKE  ''%''))) AND ((( searches.page_exclusion_filter ) ILIKE  ''%'') AND ((( searches.app_id  ) ILIKE  ''%'') AND (( searches.page_section ) ILIKE  ''%'')) AND ((( searches.page_subsection  ) ILIKE  ''%'') AND ((( COALESCE(cmslite_themes.theme_id,'''')  ) ILIKE  ''%'') AND (searches.terms ) IS NOT NULL)))
    GROUP BY
        1,
        2,
        3,
