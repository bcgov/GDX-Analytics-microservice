...
(
  ...
  WHERE status = ''new'' -- for now, report only on new appointments, not updates
    AND timestamp > DATEADD(day,-1, DATE_TRUNC(''day'',GETDATE()) )
  ...
  final_selections AS ( -- this is to assign the location and service as the latest selection
    SELECT
      ...
      appointment_id,
      client_id,
      ROW_NUMBER() OVER (PARTITION BY session_id ORDER BY step_order DESC, timestamp DESC) AS session_id_ranked
    FROM raw_list
  )
  SELECT
    rl.session_id,
  ...
  FROM raw_list AS rl
  JOIN final_selections AS fs
    ON fs.session_id = rl.session_id
    AND fs.session_id_ranked = 1
