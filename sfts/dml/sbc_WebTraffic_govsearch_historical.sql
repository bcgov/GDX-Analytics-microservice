SELECT * FROM (
    -- based off the explore https://analytics.gov.bc.ca/explore/snowplow_web_block/searches?qid=X6UVfbdwxJYEsAbssOaZUy&origin_space=37&toggle=fil,pik,vis
SELECT
...
WHERE LENGTH(searches.terms ) <> 0 AND (COALESCE(cmslite_themes.topic, ''(no topic)'') ) = ''Service BC'' AND ((searches.terms ) IS NOT NULL AND ((searches.node_id) IS NOT NULL AND (searches.page_urlhost ) IS NOT NULL)) AND ((searches.page_exclusion_filter) IS NOT NULL AND (searches.app_id ) IS NOT NULL AND ((searches.page_section) IS NOT NULL AND ((searches.page_subsection ) IS NOT NULL AND (COALESCE(cmslite_themes.theme_id,'''') ) IS NOT NULL)))
GROUP BY
    1,
    2,
    3
