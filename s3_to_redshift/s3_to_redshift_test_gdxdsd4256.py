...
test_4256 = False if 'test_4256' not in data else data['test_4256']
...
""".format(dbtable, table_name)
    ...
    # if test_4256 is set to true, then the query is run against the test table

    if test_4256 and outfile == goodfile:
        with open(sql_query) as f:
            test_4256_query = f.read()

        with spdb.connection as conn:
            with conn.cursor() as curs:
                try:
                    curs.execute(test_4256_query.strip())
                except Exception as err:
                    outfile = badfile
                    logger.error(
                        "Loading TEST to RedShift failed.")
                    spdb.print_psycopg2_exception(err)
                else:
                    logger.info(
                        "Loaded TEST to RedShift successfully")
                    outfile = goodfile
